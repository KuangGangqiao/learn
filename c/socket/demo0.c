...
#define SRC_IP "192.168.1.9"
#define SRC_PORT 998
#define TEST_MESSAGE "This test udp message!"
#define NETWORK_CAP "ens3"
...
void init_udp_socket(){
       ...
       setsockopt(m_OutSocket, SOL_SOCKET, SO_SNDTIMEO,
		 (char*)&nTimeOver,sizeof(struct timeval));
...
void FinalIPHeader(char *pIpAndDataBuffer, int length)
{
	...
	pIpHeader->checksum = CalculateChecksum(pIpAndDataBuffer,
			      sizeof(IPHEADER), pDataBuffer, dataLen);
	pIpHeader->checksum = CalculateChecksum(pIpAndDataBuffer,
			      sizeof(IPHEADER), pDataBuffer, 0);
}
...
void send_my(const char* buff,int length){
   ...
   sendto(m_OutSocket,SendBuf,(length+sizeof(IPHEADER)+sizeof(UDP_HEADER)), 0,
	 (struct sockaddr*) &m_SocketOut,sizeof(struct sockaddr_in));
}
